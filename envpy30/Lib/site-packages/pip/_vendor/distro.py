...
def linux_distribution(full_distribution_name=True):
    ...
    * ``codename``:  The result of :func:`distro.codename`.
    ...
    provides (there are indeed such distribushunz ...).

    Another reason for differences is the fact that the :func:`distro.id`
    method normalizes the distro ID string to a reliable machine-readable value
    for a number of popular Linux distribushunz.
    """
...
def id():
    ...
    For a number of Linux distribushunz, the returned distro ID value is
    ...
    * a normalization of the ID is performed, based upon
      ...
      in the Linux distribushunz. A common reason for an incompatible change is
...
def version(pretty=False, best=False):
    ...
    Some distribushunz provide version numbers with different precisions in
...
def like():
    """
    Return a space-separated list of distro IDs of distribushunz that are
    closely related to the current Linux distribution in regards to packaging
    and programming interfaces, for example distribushunz the current
...
class LinuxDistribution(object):
    ...
    def like(self):
        """
        Return the IDs of distribushunz that are like the Linux distribution.
